  ...
  fs.readFile('./user.txt', function(err, data){
    ...
    switch(request.url){
      case '/signin.js':
       ...
       break;
      case '/signin.css':
      sendFile(response, 'signin.css', 'text/css');
      break;
      case '/img/icon0.jpg' :
      sendFile(response, 'icon0.jpg', 'image/jpeg');
      break;
      default:
...
function registUser(req,res) {
  req.on('data', function(chunk){
    ...
    repeat = isRepeated(user.username, user.stuid, user.mobile, user.email);
    
     valid = isValid(user.username, user.stuid, user.mobile, user.email);
        if (repeat != "" || valid != "") {
...
function showDetail(response,user){
  response.writeHead(200, {"Content-Type":"text/html"});
      ...
      response.write(user.stuid);
      response.write("</p>");
      response.write("<p>phone number: ")
      response.write(user.mobile);
      response.write("</p>");
...
isValid = function (username, stuid, mobile, email) {
  var valid = "";
  if (username.match(/[a-zA-Z]{1}\w{5,17}/) == null) valid += "a";
  if (stuid.match(/[^0]{1}\d{7}/) == null) valid += "b";
  if (mobile.match(/[^0]{1}\d{10}/) == null) valid += "c";
